...
export const downloadProduct = api<DownloadProductRequest, DownloadProductResponse>(
  ...
  async (req) => {
    ...
    if (product.priceCents === 0) {
      ...
      if (!purchase) {
        console.log("No purchase found, attempting to verify session with Stripe...");
        ...
        try {
          ...
          if (session.payment_status === "paid" && session.status === "complete") {
            ...
            if (sessionProductId === req.productId) {
              console.log("Session is valid, creating missing purchase record...");
              ...
              console.error("Product ID mismatch. Session product:", sessionProductId, "Requested product:", req.productId);
      ...
      console.log("Download authorized for session:", req.sessionId, "product:", req.productId);
    }
