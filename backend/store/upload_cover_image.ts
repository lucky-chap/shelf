...
export const uploadCoverImage = api<UploadCoverImageRequest, UploadCoverImageResponse>(
  { expose: true, method: "POST", path: "/store/upload/cover" },
  async (req) => {
    ...
    if (!ext || !allowedExtensions.includes(ext)) {
      throw APIError.invalidArgument("only image files are allowed for cover images");
    }
